...
namespace Business.ValidationRules.FluentValidation.WholeSaler
{
    public class UpdateValidator : AbstractValidator<WholeSalerUpdateDto>
    {
        public UpdateValidator()
        {
            RuleFor(r => r.Id).NotEmpty().WithMessage("Bu alan boş olamaz");
            RuleFor(r => r.Name).NotEmpty().WithMessage("Bu alan boş olamaz");
            RuleFor(r => r.AuthorizedPerson).NotEmpty().WithMessage("Bu alan boş olamaz");
