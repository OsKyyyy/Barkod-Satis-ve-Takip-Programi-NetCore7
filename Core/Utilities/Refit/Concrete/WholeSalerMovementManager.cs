...
{
    public class WholeSalerMovementManager : IWholeSalerMovement
    {
        private IWholeSalerMovement myAPI = RestService.For<IWholeSalerMovement>("http://localhost:63067/api");
        ...
        public async Task<DataResult<List<ViewModel>>> ListByWholeSalerId([Header("Authorization: Bearer")] string token, int id)
        {
            DataResult<List<ViewModel>> dataResult = new DataResult<List<ViewModel>>();

            try
            {
                dataResult = await myAPI.ListByWholeSalerId(token, id);
