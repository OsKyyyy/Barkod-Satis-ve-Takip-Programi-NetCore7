...
{
    public class CustomerMovementManager : ICustomerMovement
    {
        private ICustomerMovement myAPI = RestService.For<ICustomerMovement>("http://localhost:63067/api");
        ...
        public async Task<DataResult<List<ViewModel>>> ListByCustomerId([Header("Authorization: Bearer")] string token, int id)
        {
            DataResult<List<ViewModel>> dataResult = new DataResult<List<ViewModel>>();

            try
            {
                dataResult = await myAPI.ListByCustomerId(token, id);
